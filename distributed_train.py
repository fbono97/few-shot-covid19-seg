...
def main(args, cfg):
    ...
    # Initialise process group for distributed sychronisation
    ...
    # Wrap model around DistributedDataParallel constructor for distributed training
    ...
    # FewShotSampler draws episodes randomly for each process,
    # so a dedicated Distributed Sampler is not strictly needed 
    ...
    for epoch in range(start_epoch, cfg['max_epoch']):
        ...
        query_loss, align_loss, threshold_loss, total_loss = train(
            cfg, train_loader, model, optimizer, scheduler, criterion, epoch, logger, writer, device, local_rank
        )
        ...
        if ((epoch+1) % cfg["save_ckpt_freq"]) == 0 and local_rank == 0:
...
def train(cfg, train_loader, model, optimizer, scheduler, criterion, epoch, logger, writer, device, local_rank):
